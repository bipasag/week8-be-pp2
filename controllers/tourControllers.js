...
const getTourById = async (req, res) => {
  ...
  try {
    const user_id = req.user._id;
    const tour = await Tour.findOne({ _id: tourId, user_id }); // 👈 ownership check
    if (tour) {
...
const updateTour = async (req, res) => {
  ...
  try {
    ...
    const updatedTour = await Tour.findOneAndUpdate(
      { _id: tourId, user_id }, // 👈 ownership check
      { ...req.body },
      { new: true }
    );
    if (updatedTour) {
...
const deleteTour = async (req, res) => {
  ...
  try {
    const user_id = req.user._id;
    const deletedTour = await Tour.findOneAndDelete({ _id: tourId, user_id }); // 👈 ownership check
    if (deletedTour) {
